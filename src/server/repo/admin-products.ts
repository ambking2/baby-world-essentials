...
export async function adminListProducts(
  filters: { q?: string; categoryId?: number; page?: number } = {},
): Promise<{
  ...
  const [total, rows] = await Promise.all([
    count(`SELECT COUNT(*) AS c FROM products p WHERE ${where}`, ...params),
    all<{
      id: number;
      code: string;
      slug: string;
...
export async function adminProductForm(id: number): Promise<ProductFormValues | null> {
  const row = await one<{
    id: number;
    code: string;
    slug: string;
  ...
  const [images, attributes, variantRows] = await Promise.all([
 ...
 *
 * روی D1 تراکنش تعاملی وجود ندارد؛ ابتدا ردیف محصول درج/به‌روز می‌شود تا
 * شناسه در دسترس باشد، سپس تمام جداول فرزند در یک batch اتمیک نوشته می‌شوند.
 */
export async function adminSaveProduct(values: ProductFormValues): Promise<number> {
  const now = nowIso();
  ...
  if (values.images) {
    writes.push(statement("DELETE FROM product_images WHERE product_id = ?", productId));
  ...
  if (values.attributes) {
    writes.push(statement("DELETE FROM product_attributes WHERE product_id = ?", productId));
  ...
  if (values.variants) {
    writes.push(statement("DELETE FROM product_variants WHERE product_id = ?", productId));
...
export async function adminToggleProduct(id: number, isActive: boolean): Promise<void> {
  await run(
...
export async function adminSetStock(id: number, stock: number): Promise<void> {
  await run(
    "UPDATE products SET stock = ?, updated_at = ? WHERE id = ?",
    Math.max(0, Math.round(stock)),
    nowIso(),
    id,
  );
}

export async function adminSetPrice(id: number, price: number): Promise<void> {
  await run(
 ...
 */
export async function adminSetDiscount(
  id: number,
...
export async function adminListReviews(
  ...
  const rows = status
    ? await all<AdminReviewRow>(`${base} WHERE r.status = ? ORDER BY r.id DESC`, status)
    : await all<AdminReviewRow>(`${base} ORDER BY r.id DESC`);
...
export async function adminListCoupons(): Promise<Array<AdminCoupon>> {
